...
- **Status (2026-09-25):** v1 is live. Deployed to the owner's Droplet (`cockwomble`, Ubuntu 20.04, Docker upgraded from 19.03 to 28.1.1 via Docker's focal repo) at `/opt/newsbot` as a git clone; first production digest posted 2026-09-25 09:36 PT with status ok. Backup systemd timer installed (08:30 America/Los_Angeles; first backup verified). Known: IGN returns 403 to the Droplet's IP (datacenter block), candidate for removal from prod config. Still to do: second-account permission check in the prod guild; review the Dependabot PRs (note: anthropic 0.125 -> 1.x is a major SDK upgrade, httpx -> httpx2); upgrade the Droplet's OS off Ubuntu 20.04 (out of standard support); delete `/opt/newsbot.pre-clone` once satisfied.
