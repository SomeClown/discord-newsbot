...
- **Status (2026-09-25):** Production runs **v1.1.1** (pinned via `TAG=1.1.1` in the Droplet's `.env`) on `cockwomble` (Ubuntu 20.04, Docker 28.1.1) at `/opt/newsbot`, a git clone updated with `scripts/deploy.sh`. v1.1.0 = Anthropic SDK 1.8.0 + IGN removed + upgrade tooling; v1.1.1 = `/newsbot status` shows only configured sources. Backups: systemd timer at 08:30 America/Los_Angeles plus one per deploy. The dev bot runs on the owner's Mac against the private test guild (`.env.dev`, `config.dev.yaml`). Releases: wait for the tag's CI build before running `deploy.sh` (else `manifest unknown`). Still to do: second-account permission check in the prod guild; delete `/opt/newsbot.pre-clone`; upgrade the Droplet off Ubuntu 20.04 (snapshot first); Dependabot will propose a `pydantic` bump (pydantic-core alone can't be bumped). Future enhancements (owner, 2026-09-25): (1) write a Privacy Policy and (2) Terms of Service for the bot, and host them at stable public URLs (candidates: GitHub Pages from this public repo, or the owner's Droplet domain) so they can be linked in the Discord Developer Portal (General Information: Terms of Service URL / Privacy Policy URL) for app verification.
