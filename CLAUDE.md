...
- **Status (2026-09-25):** v1 is live. Deployed to the owner's Droplet (`cockwomble`, Ubuntu 20.04, Docker upgraded from 19.03 to 28.1.1 via Docker's focal repo) at `/opt/newsbot` as a git clone; first production digest posted 2026-09-25 09:36 PT with status ok. Backup systemd timer installed (08:30 America/Los_Angeles; first backup verified). IGN removed from the prod config (403s the Droplet's datacenter IP; its source_health row was deleted by hand, since `/newsbot status` lists every recorded source, not just configured ones: follow-up). Dependabot round 1 done (Actions bumped, setuptools merged, pydantic-core closed). Still to do: second-account permission check in the prod guild; deploy the anthropic 1.8.0 migration (PR #9, merged 2026-09-25 after a live test-guild preview) to prod; upgrade the Droplet's OS off Ubuntu 20.04 (out of standard support); delete `/opt/newsbot.pre-clone` once satisfied.
