...
- **Status (2026-09-24):** M1 and M2 complete; M3 steps 18–21 complete. Pushed to github.com/SomeClown/discord-newsbot (private repo); CI green; image `ghcr.io/someclown/discord-newsbot:latest` is public (anonymous pull verified). MIT license, (c) Teren Bryson. Paused before step 22 (first Droplet deploy), which the owner will do with a walkthrough (owner runs the commands; owner places the prod `.env` themselves). Step 22 must include the second-account permission check, which hasn't been run yet. Follow-up: bump GitHub Actions off Node 20 (checkout@v4, setup-python@v5, docker/* actions).
