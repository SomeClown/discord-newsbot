...
- **Status (2026-09-24):** M1 and M2 complete; M3 in progress. Step 17 walkthrough passed in the test guild (the second-account permission check hasn't been run yet; do it at Checkpoint H). QA step 20's findings are fixed (userinfo/homograph URL rejection, model-text URL stripping, resumable publisher, stuck-pending recovery, UTF-16 embed limits, compose env_file split, plus the cheap LOW-severity batch). The Docker image builds and comes up `healthy` in the test guild. The owner re-reviewed a `/newsbot preview` after the prompt change and approved it. Not yet done: push to GitHub + CI (Checkpoint F), first deploy to the Droplet (Checkpoint H, step 22).
