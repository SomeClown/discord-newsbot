...
- **QA item 7: option A trust-gated pings; roundups >5 codes silent — owner, 2026-09-25.** Every new code still posts; a batch pings only if at least one candidate queued to post came from a source whose trust is in the new `AlertsCfg.ping_trust` (default `["official", "press"]`) -- a community-only batch posts quietly and spends nothing against the daily cap. An item naming more than the new `AlertsCfg.max_codes_per_item` (default 5) distinct codes is a roundup/megathread; its codes are recorded silently as a new `alerted_codes.status` value, `'roundup'` (migration 002 edited directly, since it was still unreleased), and never alert -- unless the same code also shows up in a normal item in the same run, in which case it's judged entirely by that normal item.
